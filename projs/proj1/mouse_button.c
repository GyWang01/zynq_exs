...
void test_send_button(int uinput_fd)
{
        ...
        if (read_gpio(GPIO_BTN_LEFT) == 0) {
            send_mouse_button_event(uinput_fd, BTN_LEFT, 1); // 按下左键
            usleep(100000); // 等待100ms
            send_mouse_button_event(uinput_fd, BTN_LEFT, 0); // 松开左键
        }

        if (read_gpio(GPIO_BTN_RIGHT) == 0) {
            send_mouse_button_event(uinput_fd, BTN_RIGHT, 1); // 按下右键
            usleep(100000); // 等待100ms
            send_mouse_button_event(uinput_fd, BTN_RIGHT, 0); // 松开右键
        }
}
