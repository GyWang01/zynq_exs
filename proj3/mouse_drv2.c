...
static void mpu_mouse_poll_work(struct work_struct *work)
{
    ...
    raw_accel_x = (buf[0] << 8) | buf[1];
    i2c_smbus_read_i2c_block_data(mydevice->client, 0x3D, 2, buf);
    raw_accel_y = (buf[0] << 8) | buf[1];

    // Convert raw data to fixed-point g units (Q15 format)
    int accel_x_fixed = (raw_accel_x * 10) / ACCEL_SCALE; // 10 is the scale factor
    int accel_y_fixed = (raw_accel_y * 10) / ACCEL_SCALE; // 10 is the scale factor

    // Apply low-pass filter
    filtered_x = low_pass_filter(mydevice->prev_accel_x, accel_x_fixed);
    filtered_y = low_pass_filter(mydevice->prev_accel_y, accel_y_fixed);
